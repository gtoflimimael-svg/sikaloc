 ...
 *
 * `mentionsConformes: false` : cet exemple-ci est celui du plan GRATUIT, sans
 * la mention du droit de timbre. Son jumeau `/exemple-quittance` montre la
 * version Standard, qui la porte.
 *
 * Deux exemples plutôt qu'un seul parce qu'un seul mentait par omission : le
 * visiteur qui clique depuis une surface vendant le plan Gratuit voyait un
 * document Standard, et croyait donc que son futur compte produirait la
 * mention du droit de timbre. Chaque plan montre désormais ce qu'il produit
 * réellement.
 */
...
export async function GET() {
  const pdf = await rendrePdf({
    numeroDocument: 'EXEMPLE-GRATUIT',
    type: 'Quittance',
  ...
  return new NextResponse(pdf as unknown as BodyInit, {
    headers: {
      ...
      'Content-Disposition': 'inline; filename="exemple-quittance-plan-gratuit.pdf"',
